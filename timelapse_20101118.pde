...
int i = 1;
int framerate = 30;
int diff = 60 * 1000; // Here "1" is our desired interval time in ms * 1000 = seconds.
//1000msec = 1sec.
//I separated the "*1000" just to make it easier for myself...
float m = millis();

int STOPDAY = 19; //this sets the day of the month at which we stop
int STOPHOUR = 20;  //change this to be the time at which you want to stop the recording. You can edit the code to stop at whatever time you like.
int STOPMINUTE = 0;  //change this to set the minutes at which recording will stop

int STARTDAY = 19; //this sets the day of the month at which we start
int STARTHOUR = 5; //this sets the hour at which we start
...
void setup() {
  ...
  // camFake = new Capture(this,640,480,devices[4]);
  cam = new Capture(this, 640, 480, devices[4]);
  //cam.settings();
